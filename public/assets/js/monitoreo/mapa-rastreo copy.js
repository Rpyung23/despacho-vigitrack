...
function is_transmitiendo(fechaUltiMoni) {
    ...
    if (convert_diferencia_from_minutes >= -1000 && convert_diferencia_from_minutes <= 5) {
        bandera = 1 /**esta transmitiendo**/
    } else if (convert_diferencia_from_minutes > 5 && convert_diferencia_from_minutes < 15) {
...
let rastreo = () => {
    ...
    var json_body = {
        db_name: "uambatena",
        script: `select M.CodiVehiMoni,R.DescRuta,M.PlacVehiMoni,M.AlarEnceChapMoni,M.AlarFuerRutaMoni,M.AlarAnteGPSDescMoni,M.EvenAnteGPSConeMoni,M.EvenAlimBateExteMoni,M.Odometro,M.EstaSaliMoni,M.UltiFechMoni,M.UltiLugaMoni,M.UltiLatiMoni,M.UltiLongMoni,M.UltiRumbMoni,
         M.UltiVeloMoni from monitoreo as M join vehiculo as V on M.CodiVehiMoni = V.CodiVehi join ruta as R on M.LetrRutaMoni = R.LetrRuta
         where V.idEstaVehi = 1 order by  CONVERT(M.CodiVehiMoni, SIGNED INTEGER)  ASC`
    }
...
function createListRatreo_AndListHtml(dataset) {
    for (var j = 0; j < dataset.length; j++) {
        ...
        //(bandera_time_trasmicion_, velo, in_ruta, is_encendido, alar_ante_gps)
        ...
        marker_ = new google.maps.Marker({
            ...
            icon: get_image_marker(bandera_time_trasmicion, dataset[posInfo].ultivelomoni,
                dataset[posInfo].alarfuerrutamoni, dataset[posInfo].alarencechapmoni, dataset[posInfo].alarantegpsdescmoni) /*"../public/assets/images/monitoreo/bus_online.svg"*/
        })
        ...
        google.maps.event.addListener(marker_, 'click', (function(marker, posInfo) {
            return async function() {
                ...
                var geo = await geocodeLatLng(geocoder, dataset[posInfo].ultilatimoni, dataset[posInfo].ultilongmoni)
                ...
                <strong>Posición</strong> : <strong><label>${geo}</label></strong><br>
                <strong>Velo : </strong>${dataset[posInfo].ultivelomoni} km/h<br>
                <strong>Km Recorridos : </strong>${dataset[posInfo].odometro ==null ? 0 : dataset[posInfo].odometro } km/h<br>
                <strong>Evento : ${get_EventoString(bandera_time_trasmicion, dataset[posInfo].ultivelomoni,
                    dataset[posInfo].alarfuerrutamoni, dataset[posInfo].alarencechapmoni, dataset[posInfo].alarantegpsdescmoni)} </strong><br>
                <strong>Ruta : </strong>${dataset[posInfo].descruta}
                `
                ...
                list_objs_unidades_monitoreo[posInfo].infowindow = infowindow_

                console.log(list_objs_unidades_monitoreo[posInfo].infowindow)
        ...
        var obj_uni_monit = {
            ...
            alarencechapmoni: dataset[posInfo].alarencechapmoni,
            ...
            descruta: dataset[posInfo].descruta,
            marker: marker_,
        ...
        if (alar_ante_gps != null && alar_ante_gps) {
            ...
            if (in_ruta != null && in_ruta) {
                ...
                if (velo == 0) {
                    ...
                    // console.log("bandera_time_trasmicion_ > = 0 && parseFloat(bandera_time_trasmicion_) <= 5 " + bandera_time_trasmicion_)
                    if (bandera_time_trasmicion_ == 1) {
                        //   console.log("online")
                        return { url: "../public/assets/images/monitoreo/autobus_online.png", }
                    } else if (bandera_time_trasmicion_ == 2) {
                        //  console.log("now")
...
function get_EventoString(bandera_time_trasmicion_, velo, in_ruta, is_encendido, alar_ante_gps) {



    if (is_encendido == null || !is_encendido) {
        // console.log("alar_ante_gps " + alar_ante_gps)
        if (alar_ante_gps != null && alar_ante_gps) {
            return "Unidad en Alerta"
        } else {
            if (in_ruta != null && in_ruta) {
                return "Fuera de Ruta"
            } else {
                if (velo == 0) {
                    return "Unidad Detenida"
                } else {
                    ...
                    if (bandera_time_trasmicion_ == 1) {
                        ...
                        //  console.log("full")
                        return "Sin Transmición Full."
                    }

                }
            }

            return "Unidad Encendida"
        }
    } else {
        return "Unidad Apagada"
    }
...
async function geocodeLatLng(geocoder, lat, lng) {
    ...
    try {
        ...
        if (datos.results[0]) {
            var respuesta = response.results[0].formatted_address
